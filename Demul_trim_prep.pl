...
while (<INBC>) {
	...
	for ( my $i = 0; $i < scalar(@line); $i++ ) {
		my $key   = $cols[$i];
		my $value = $line[$i];
		$mapping{ $line[0] }{$key} = $value;
	}

	# insert all single-error barcodes
	for ( my $i = 0; $i < length( $mapping{ $line[0] }{"BarcodeSequence"} ); $i++ ) {
		my @chars = ( "A", "C", "G", "T", "N" );
		my $s = $mapping{ $line[0] }{"BarcodeSequence"};
		foreach my $ck (@chars) {
			...
			print STDERR "Barcode collision! $s => ".$mapping{ $line[0] }{"BarcodeSequence"}." was already defined as $barcode_forward{$s}!!\n"
			  if defined $barcode_forward{$s} && $s ne $mapping{ $line[0] }{"BarcodeSequence"};
			$barcode_forward{$s} = $line[0].".FBC";
		}
	}
	for ( my $i = 0; $i < length( $mapping{ $line[0] }{"ReverseBarcode"} ); $i++ ) {
		my @chars = ( "A", "C", "G", "T", "N" );
		my $s = $mapping{ $line[0] }{"ReverseBarcode"};
		foreach my $ck (@chars) {
			substr( $s, $_, 1 ) =~ s/[ACGT]/$ck/ for $i;
			print STDERR "Barcode collision! $s => ".$mapping{ $line[0] }{"ReverseBarcode"}." was already defined as $barcode_reverse{$s}!!\n"
			  if defined $barcode_reverse{$s} && $s ne $mapping{ $line[0] }{"ReverseBarcode"};
			$barcode_reverse{$s} = $line[0].".RBC";
		}
	}
	$mapping{ $line[0].".FBC" }{ $line[0].".RBC" } = $line[0];
	$output_filehandles_full{ $line[0] } = new IO::Zlib;
	$output_filehandles_full{ $line[0] }->open( "$output_dir/$out_file_core"."_$line[0].fastq.gz", "wb9" );
	open( $output_filehandles_1{ $line[0] }, ">$output_dir/$out_file_core"."_$line[0]"."_1".".fastq" ) unless $skip_merge;
	open( $output_filehandles_2{ $line[0] }, ">$output_dir/$out_file_core"."_$line[0]"."_2".".fastq" ) unless $skip_merge;
}
close(INBC);
